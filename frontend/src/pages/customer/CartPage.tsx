...
const CartPage: React.FC = () => {
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      const orderData = {
        id: uniqueOrderId, // ✅ Required by backend
        customerId: "681521470a1fe0512b136ab5", // ✅ Use valid ID from your test
        restaurantId: localStorage.getItem('restID'),
        items: cartItems.map((cartItem, index) => ({
          itemId: `ITEM-${String(index + 1).padStart(3, '0')}`, // ITEM-001 etc.
          name: cartItem.item.name,
          price: cartItem.item.price,
          quantity: cartItem.quantity,
          totalPrice: cartItem.item.price * cartItem.quantity
        ...
        totalPrice: cartTotal,
        paymentStatus: "PAID", // ✅ Required by backend
        orderStatus: "PACKED", // ✅ Required value (not "PENDING")
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div>
                <div className="bg-white rounded-lg shadow-md p-6 sticky top-24">
                  ...
                  <button 
                    onClick={handleProceedToPayment}
                    disabled={!deliveryAddress.trim() || !customerEmail.trim() || !phoneNumber.trim() || isProcessing}
                    className="w-full py-3 bg-orange-500 text-white rounded-md hover:bg-orange-600 transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed"
                  >
